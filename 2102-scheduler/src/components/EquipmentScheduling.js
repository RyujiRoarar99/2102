...
class EquipmentScheduling extends Component {
    constructor(props) {
        ...
        //get all slots
        Axios.post("http://localhost:3001/GetSlotsFromToday").then(
            (response) => {
        ...
        Axios.post("http://localhost:3001/GetLoggedEquipment").then(
            (response) => {
                if (response.data.length) {
                    let array = [];
                    response.data.map((e) =>
                        array.push({
                            allDay: true,
                            title: e.type + " " + e.serial_no,
                            start: new Date(e.date_to_sample),
                            id: e.serial_no,
                            data: [e.type, e.id, e.date_to_sample],
                        })
                    );
    ...
    handleChange = (event) => {
        ...
        //get all equipment
        Axios.post("http://localhost:3001/GetAllEquipmentFiltered", {
            filterSerial: event.target.value,
        }).then((response) => {
            if (response.data.length) {
                this.setState({ equipment: response.data });
            } else {
    ...
    dropController = (event) => {
        ...
        if (dateExist) {
            ...
            if (
                this.state.scopeSlots[index][1] !==
                this.state.scopeSlots[index][2]
            ) {
                let urlString = "";
                ...
                Axios.post("http://localhost:3001/AddScopeFilled", {
                    filled: item[2],
                    date: date,
                }).then((response) => {});
                Axios.post(urlString, { scope_id: data[0], date: date }).then(
                    (response) => {}
                );
            } else {
                alert(
                    "Slot is full! Either delete a scope or expand the number of slots!"
                );
                event.remove();
            }
        } else {
            alert(
                "No slot assigned for this date! Please go to slot allocation to allocate a slot!"
            );
            event.remove();
        }
    };
        ...
        Alert.fire({
          ...
          ` +
            ...
            if (result.value) {
                ...
                Axios.post(urlString, { scope_id: data[1], date: date }).then(
                    (response) => {}
                );
                    ...
                    <Row>
                        ...
                        <Col id="eventTable" lg={3}>
                            ...
                            >
                                ...
                                <tbody>
                                    {this.state.scopeSlots.map((slot) => (
                                        <tr>
                                            <td>{slot[0]}</td>
